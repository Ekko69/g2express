...
//
  ...
  Future<List<Address>> findAddressesFromQuery(String address) async {
    ...
    if (LocationService.currenctAddress != null) {
      myLatLng =
          "${LocationService?.currenctAddress?.coordinates?.latitude},${LocationService?.currenctAddress?.coordinates?.longitude}";
    }
    ...
    final apiKey = AppStrings.googleMapApiKey;
    final apiResult = await getExternal(
      "https://maps.googleapis.com/maps/api/place/textsearch/json?query=$address&key=$apiKey&location=$myLatLng",
    );
    //
    if (apiResult.statusCode == 200) {
      //
